...
class Report extends CI_Controller {
  ...
  function project_status_rpt(){
    ...
    if($this->session->userdata('org_type')==1){
      $data['rptname'] = "Project Status Report";
    }else{
      $data['rptname'] = $this->session->userdata('org_name')." Project Status Report";
    }
  ...
  function gn_divwise_benificiaries(){
    $this->is_logged();
    $this->is_privileged(36,$this->session->userdata('user_level'),1);
    $data['titlename'] = "GN division wise Beneficiaries";

    $getgndivisions = $this->common_model->get_gn_divsions();
    $mainarr = [];
    foreach($getgndivisions as $gndv){
      $subarr = array(
       'gndivname' => $gndv->gn_division,
       'count' => $this->common_model->get_gn_divsions_wise_benificiaries_count($gndv->gn_div_id)
      );
      array_push($mainarr,$subarr);
    }
    
    $data['benificarycount'] = $mainarr;
    
    $data['notificationdata'] = $this->get_notifications();
    $this->load->view('reports/gn_divisonwise_benificiaries_viw',$data);
  }
